    ...
    async def play(self, ctx, *, query):
        ...
        if not player.is_connected:
            ...
            if not permissions.connect or not permissions.speak:
                ...
                return await ctx.send(f"I can't seem to play music in **{ctx.author.voice.channel.name}**... sad")
        ...
        if tracks[0]['info']['length'] > 900000 and not tracks[0]['info']['isStream']:

            return await ctx.send(f"Song is too long, sorry\n{mstomin(tracks[0]['info']['length'])}m / {mstomin(900000)}m")
